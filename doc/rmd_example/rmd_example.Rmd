...
knitr::kable(horses_sd_table, echo = FALSE, caption = "Standard Deviations of Horse Population per Province")
```
...
Additionally, note that in \@ref(tab:sd-table) we consider the sample standard deviation of the number of horses during the same time span as \@ref(fig:horse-pop).


```{r horse-pop-large-sd, out.width="50%", echo = FALSE, fig.cap = "Horse populations for the province with the largest standard deviation"}
knitr::include_graphics("../../results/horse_pop_plot_largest_sd.png")
```

In \@ref(fig:horse-pop-large-sd) we zoom in and look at the province of ?, which had the largest spread of values in terms of standard deviation.
